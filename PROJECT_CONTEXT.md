...
- 数据持久化到 JSON
- 支持中英文界面，并根据系统语言自动切换
- 支持账号列表按最近使用、5E 分段、封号/冻结状态排序
- 支持自定义账号分组、分组筛选和批量设置分组
- 支持账号列表的搜索词、状态筛选、分组筛选和排序方式持久化到 `settings.json`
- Steam 登录名和 Steam 密码是核心必填信息；5E 昵称和 5E 分段是可选信息，5E 昵称会直接显示在账号列表中；5E 分段按 S > A++ > A+ > A > B++ > B+ > B > C++ > C+ > C > D 排序
- 新账号和空分段默认是 `未定级`；赛季重置可一键把所有账号设为未定级，并把已定级账号的上赛季分段追加到备注
- Steam 登录尝试成功启动后会自动更新 `last_login`，用于最近使用排序
- 主列表和已有账号详情弹窗都可以直接发起 Steam 登录；详情弹窗会使用当前表单里的登录名和密码
- 主要面向 Windows 环境
...
- `phone`
  电话或其他辅助字段
- `group_name`
  用户自定义分组名；为空表示未分组。界面支持按分组筛选，也支持对选中账号批量设置分组
- `five_e_nickname`
  5E 昵称；文本导入时会从 `昵称` 或非手机号的 `5E账号` 解析出来，手动编辑时也可直接填写。旧数据如果没有该字段，界面会从备注中的 `5E昵称: ...` / `5E Nickname: ...` 兜底显示
- `five_e_rank`
...
- 账号列表新增分组列，账号详情里可填写自定义分组名，空分组显示为未分组
- 账号列表顶部新增分组筛选；批量操作区可以把选中账号设置到指定分组，也可以清空分组
- 账号列表新增 5E 昵称列，优先显示 `five_e_nickname` 字段；旧账号没有该字段时会从备注中的 `5E昵称: ...` / `5E Nickname: ...` 兜底读取
- 新建/编辑账号弹窗增加独立的 5E 昵称输入项
- 已有账号详情弹窗增加“登录Steam”按钮，不需要关闭弹窗回到主界面再登录
- 文本导入会把解析出的 5E 昵称写入独立的 `five_e_nickname` 字段
- 搜索词、状态筛选、分组筛选和排序方式会保存到 `settings.json`，下次启动自动恢复
- 状态筛选、分组筛选和排序保存内部 key，不依赖中英文显示文案
...
- 主界面工具栏的“登录Steam”按钮，会使用当前选中账号已保存的 `login_name` 和 `password`
- 已有账号详情弹窗底部的“登录Steam”按钮，会使用弹窗当前表单里的 `login_name` 和 `password`，不需要先关闭弹窗

登录流程大致如下：

1. 获取目标账号或详情弹窗当前表单里的 `login_name` 和 `password`
2. 尝试找到或选择 `steam.exe`
